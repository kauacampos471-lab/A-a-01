...
const Auth = (() => {
  ...
  function getUsers() { return Users.cache.length ? Users.cache : [DEFAULT_OWNER]; }
...
const DB = (() => {
  ...
  return {
    ...
    addEstoqueItem(item) {
      ...
      return fdb.collection('estoque').add(item);
    },
    updateEstoqueItem(id, patch) { fdb.collection('estoque').doc(id).update(patch); },
    deleteEstoqueItemWithUndo(id, label) {
      pendingDelete[id] = true; notify();
      const timer = setTimeout(() => { fdb.collection('estoque').doc(id).delete(); delete pendingDelete[id]; }, 5000);
    ...
    adjustEstoqueQtd(id, delta) {
      ...
      if (item) {
        ...
        fdb.collection('estoque').doc(id).update({ quantidade: novaQtd });
      }
    },
    ...
    addProduto(p) {
      ...
      return fdb.collection('produtos').add(p);
    },
    updateProduto(id, patch) { fdb.collection('produtos').doc(id).update(patch); },
    deleteProdutoWithUndo(id, label) {
      pendingDelete[id] = true; notify();
      const timer = setTimeout(() => { fdb.collection('produtos').doc(id).delete(); delete pendingDelete[id]; }, 5000);
      undoToast(`"${label}" excluído.`, () => { clearTimeout(timer); delete pendingDelete[id]; notify(); });
    },
    ...
    addVenda(v) {
      ...
      return fdb.collection('vendas').add(v);
    },
...
const PDV = (() => {
  ...
  function getCombos() {
    let combos = JSON.parse(localStorage.getItem(COMBOS_KEY) || 'null');
    if (!combos) {
      combos = [
        { id: 'C1', nome: 'Combo Tradicional', preco: 0, itens: [
          { nome: 'Granola (adicional)', preco: 2 }, { nome: 'Morango (adicional)', preco: 3 }
        ]},
        { id: 'C2', nome: 'Combo Chocolate', preco: 0, itens: [
          { nome: 'Nutella (adicional)', preco: 5 }, { nome: 'Granola (adicional)', preco: 2 }
        ]}
      ];
  ...
  function renderProductGrid() {
    ...
    grid.innerHTML = '';
    let items = [];
    if (currentCat === 'produtos') items = DB.getProdutos();
    else if (currentCat === 'extras') items = getExtras();
    else items = getCombos();
    ...
    items.forEach(p => {
      ...
      const precoLabel = currentCat === 'combos' ? brl((p.itens||[]).reduce((s,i)=>s+i.preco,0)) : brl(p.preco);
      btn.innerHTML = `<span>${p.nome}</span><span>${precoLabel}</span>${inCart ? `<span class="qty-badge">x${inCart.qtd}</span>` : ''}`;
      btn.addEventListener('click', () => {
        if (currentCat === 'combos') addComboToCart(p);
        else addToCart(p, currentCat === 'produtos' ? 'produto' : 'extra');
      });
  ...
  function addComboToCart(combo) {
    (combo.itens || []).forEach(it => {
      const existing = cart.find(c => c.nome === it.nome && c.tipo === 'extra');
      if (existing) existing.qtd += 1;
      else cart.push({ refId: 'combo-' + it.nome, tipo: 'extra', nome: it.nome, preco: it.preco, qtd: 1, obs: 'Combo: ' + combo.nome, custo: 0 });
    });
    toast(`${combo.nome} adicionado 🍇`);
    renderAll();
  }
  ...
  function renderBag() {
    ...
    container.querySelectorAll('[data-plus]').forEach(b => b.addEventListener('click', () => { cart[b.dataset.plus].qtd++; renderAll(); }));
    container.querySelectorAll('[data-minus]').forEach(b => b.addEventListener('click', () => {
      const i = b.dataset.minus; cart[i].qtd--; if (cart[i].qtd <= 0) cart.splice(i, 1); renderAll();
    }));
  }
  ...
  function printReceipt(venda) {
    ...
    const itensHtml = venda.itens.map(i => `<div class="p-line"><span>${i.qtd}x ${i.nome}</span><span>${brl(i.preco * i.qtd)}</span></div>`).join('');
    area.innerHTML = `
      <h2>BS AÇAITERIA</h2>
  ...
  function finalize() {
    ...
    const { subtotal, desconto, total } = renderComanda();
    const custoTotal = cart.reduce((s, i) => s + (i.custo || 0) * i.qtd, 0);
    const venda = {
      itens: cart, subtotal, desconto, total, custoTotal, lucro: total - custoTotal,
    ...
    cart.forEach(item => {
      if (item.tipo === 'produto') {
        ...
        if (produto && produto.receita) produto.receita.forEach(r => DB.adjustEstoqueQtd(r.estoqueId, -(r.qtd * item.qtd)));
      }
    });
  ...
  function init() {
    ...
    document.getElementById('pdv-desconto').addEventListener('input', renderComanda);
    document.getElementById('pdv-desc-tipo').addEventListener('change', renderComanda);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fdb.collection('estoque').limit(1).get()
    ...
    .catch(() => { document.getElementById('syncStatus').textContent = '⚠️ Offline — dados salvos localmente'; });
